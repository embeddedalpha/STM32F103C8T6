...
void PWM_Channel_1_Init(TIM_TypeDef *timer, uint16_t freq)
{
   if(timer == TIM1)
{
RCC ->APB2ENR |= RCC_APB2ENR_TIM1EN;
...
void PWM_Channel_2_Init(TIM_TypeDef *timer, uint16_t freq)
{
if(timer == TIM1)
{
RCC ->APB2ENR |= RCC_APB2ENR_TIM1EN;
timer->PSC = ((TIMER_CLOCK_ADVANCED) / ((freq)*(100)))-1;
}
else if(timer == TIM2)
{
RCC -> APB1ENR |= RCC_APB1ENR_TIM2EN;
timer->PSC = ((TIMER_CLOCK_GENERAL) / ((freq)*(100)))-1;
}
else if(timer == TIM3)
{
RCC -> APB1ENR |= RCC_APB1ENR_TIM3EN;
...
void PWM_Channel_3_Init(TIM_TypeDef *timer, uint16_t freq)
{
if(timer == TIM1)
{
RCC ->APB2ENR |= RCC_APB2ENR_TIM1EN;
...
void PWM_Channel_4_Init(TIM_TypeDef *timer, uint16_t freq)
{
if(timer == TIM1)
{
RCC ->APB2ENR |= RCC_APB2ENR_TIM1EN;
...
void PWM_Complementary_Ch_1_Init(uint16_t freq)
{
	...
	TIM1 -> PSC = ((TIMER_CLOCK_ADVANCED) / ((freq)*(100)))-1;
	TIM1->ARR = 100;
